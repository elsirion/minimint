...
impl ClnRpcService {
    ...
    async fn create_invoice_with_known_preimage(
        &self,
        amount_msat: u64,
        expiry_secs: u64,
        ...
        let description = match description_or {
            ...
            Some(Description::Hash(_hash)) => {
                // Currently CLN only supports direct descriptions.
                String::new()
            }
        };
...
impl GatewayLightning for ClnRpcService {
    ...
    async fn create_invoice(
        ...
        let payment_hash = if payment_hash.is_empty() {
            return self
                .create_invoice_with_known_preimage(amount_msat, expiry_secs.into(), description)
                .await;
        } else {
